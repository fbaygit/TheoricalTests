...
clearSignals = []; % all clear signals
for i = 2: 6
    clearSignals = [clearSignals; diffusion1D(.5, i, 10, x, 50)]; % Clear signs with different amplitude
end
fileName = '/Users/Felipe/Dropbox (Uniklinik Bonn)/Felipe/SNR Test/data.xls';  % URL of exported data
iter = 100; % Number of tests to do
...
% every case of estimation and their ST
...
function [allNoisedSignals, allNoisedSignalsDeconvolved, allNoises, allNoisesDeconvolved] = getAllDeconvolutions(clearSignals, kernel, iter)
    ...
    for i = 1: 5
        [noisedSignals, noisedSignalsDeconvolved, noises, noisesDeconvolved] = deconvolveSNR(clearSignals(i, :), kernel, iter);
        ...
        waitbar(i/5,f,'Deconvolving signals... ('+string(i*100/5)+'%)');
    end
    close(f)
end
...
function [noisedSignals, noisedSignalsDeconvolved, noises, noisesDeconvolved] = deconvolveSNR(clearSignal, kernel, iter)
...
function [averageCentralPeaksAccuracy, averageNeigborhoodAccuracy, averageWholeCurvesAccuracy, ...
    ...
    [i1, i2, i3, i4, i5] = deal (iter, iter*2, iter*3, iter*4, iter*5);
    ...
    for i = 1: size(allNoisedSignalsDeconvolved, 1)
        ...
        if ismembertol(i, [i1, i2, i3, i4, i5])
    ...
    for i = 1: iter: 5*iter
        ...
        waitbar(i/5*iter,f,'Calculating Standard Deviations... ('+string(i*100/5*iter)+'%)');
    end
    close(f)
end
    ...
    switch plotCase
        case "Tendency"
            ...
            for i = 1: iter: 5*iter
                ...
                plot(S)
                title(["Average of Noised signals", string(iter)+" Curves", "SNR = "+string(clearSignals(cont, 50)/0.1)], 'FontSize', 15)
                xlabel('Space')
                ylabel('Amplitude')
                grid on
                cont = cont + 1;
        ...
        case "SinglePlots"
            ...
            for i = 1: iter: 5*iter
                ...
                title(["Single Noised signal deconvolution", "SNR = "+string(clearSignals(cont, 50)/0.1)], 'FontSize', 15)
                xlabel('Space')
                ylabel('Amplitude')
